...
app.get("/students", (req: Request, res: Response) => {
  res.render("students", { title: "Students", activePage: "students", students }, (err, html) => {
    if (err) {
      console.error("Render error (/students):", err);
...
app.get("/students/:id", (req: Request, res: Response) => {
  ...
  console.log(`GET /students/${id} - ${student ? 'found' : 'not found'}`);
  res.render(
    "student-detail",
    {
    ...
    (err, html) => {
      if (err) {
        console.error("Render error (/students/:id):", err);
        return res.status(500).send("Template render error");
      }
      res.send(html);
    }
...
app.get('/debug-students-file', (req: Request, res: Response) => {
  res.sendFile(path.join(__dirname, '..', 'views', 'students.ejs'));
});

app.get('/debug-students-json', (req: Request, res: Response) => {
  res.json({ students });
});
